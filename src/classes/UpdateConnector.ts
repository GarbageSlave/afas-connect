...
export default class UpdateConnector extends Connector {
   ...
   * @returns for certain UpdateConnectors it returns an ID in either string or object, for others it returns nothing
   */
  public async insertSubUpdateMain(
    updateConnectorName: TUpdateConnectorName,
    subUpdateConnectorName: string,
    payload: Record<string, any>,
  ): Promise<any> {
    try {
      return await this.http(this.connectorUrl + updateConnectorName + '/' + subUpdateConnectorName, 'POST', payload);
    } catch (error: any) {
